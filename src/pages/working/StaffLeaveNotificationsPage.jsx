...
const StaffLeaveNotificationsPage = () => {
    ...
    const loadNotifications = async () => {
        try {
            ...
            if (!accountId) {
                // Try fallback to currentUser in localStorage
            ...
            // Keep only leave-related notifications
            ...
            console.error('[StaffLeaveNotificationsPage] load failed', err);
            setNotifications([]);
    ...
    useEffect(() => {
        // mark all as read for this account and then load
    ...
    // refresh on realtime notification that is relevant
    useEffect(() => {
        if (!signalRNotification) return;
        // If incoming notification looks like a leave notification and targets this user, reload
        try {
